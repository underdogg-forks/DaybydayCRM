...
for (const routeCase of projectRoutes) {
  guestTest(`projects guest behavior: ${routeCase.method} ${routeCase.path}`, async ({ page, request }) => {
    /* Arrange */
  ...
  authTest(`projects auth behavior: ${routeCase.method} ${routeCase.path}`, async ({ page, request }) => {
    /* Arrange */
    const routePath = interpolateRoutePath(routeCase.path);
